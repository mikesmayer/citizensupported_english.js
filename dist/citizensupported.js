...
var menu = {
    ...
    view: function view(ctrl, args) {
        return m$1('header.main-header', {
            ...
            } }, I18n$1.t('header.submit', I18nScope$16())), m$1('a.header-link.w-nav-link[href=\'/en/explore?ref=ctrse_header\']', { onclick: function onclick() {
...
var projectShareBox = {
    ...
    view: function view(ctrl, args) {
        ...
        }, '< embed >'), ctrl.displayEmbed() ? m$1('.embed-expanded.u-margintop-30', [m$1('.fontsize-small.fontweight-semibold.u-marginbottom-20', 'Insert a widget on your site'), m$1('.w-form', [m$1('input.w-input[type="text"][value="<iframe frameborder="0" height="340px" src="https://www.citizensupported.org/en/projects/' + args.project().project_id + '/embed" width="300px" scrolling="no"></iframe>"]')]), m$1('.card-embed', [m$1('iframe[frameborder="0"][height="350px"][src="/projects/' + args.project().project_id + '/embed"][width="300px"][scrolling="no"]')])]) : '', args.project().permalink ? m$1.component(facebookButton, {
            mobile: true,
...
var userContributedBox = {
    ...
    view: function view(ctrl, args) {
        ...
        return !_$1.isEmpty(collection) ? m$1('.section-one-column.u-marginbottom-30', [m$1('.fontsize-large.fontweight-semibold.u-marginbottom-30.u-text-center', title), m$1('.w-row.w-hidden-small.w-hidden-tiny.card.card-secondary', [m$1('.w-col.w-col-3', m$1('.fontsize-small.fontweight-semibold', 'Projects I supported')), m$1('.w-col.w-col-2', m$1('.fontsize-small.fontweight-semibold', 'Value of support')), m$1('.w-col.w-col-3', m$1('.fontsize-small.fontweight-semibold', 'Support Status')), m$1('.w-col.w-col-4', m$1('.fontsize-small.fontweight-semibold', 'Reward'))]), _$1.map(collection, function (contribution) {
            ...
            }))]), m$1('.fontsize-smallest', contribution.installments > 1 ? contribution.installments + ' x $ ' + contribution.installment_value + ' ' : '', contribution.payment_method == 'BoletoBancario' ? 'Bank Account' : 'Credit Card'), contributionVM.canShowReceipt(contribution) ? m$1('a.btn.btn-inline.btn-small.u-margintop-10.btn-terciary[href=\'https://www.citizensupported.org/en/projects/' + contribution.project_id + '/contributions/' + contribution.contribution_id + '/receipt\'][target=\'__blank\']', 'View Receipt') : '', contributionVM.canShowSlip(contribution) ? m$1('a.btn.btn-inline.btn-small.u-margintop-10[href=\'' + contribution.gateway_data['boleto_url'] + '\'][target=\'__blank\']', 'Print Ticket') : '', contributionVM.canGenerateSlip(contribution) ? m$1('a.btn.btn-inline.btn-small.u-margintop-10[href=\'https://www.citizensupported.org/en/projects/' + contribution.project_id + '/contributions/' + contribution.contribution_id + '/second_slip\'][target=\'__blank\']', 'Generate 2nd via') : '', m$1('.w-checkbox.fontsize-smallest.fontcolor-secondary.u-margintop-10', [m$1('input.w-checkbox-input[id=\'anonymous\'][name=\'anonymous\'][type=\'checkbox\']' + (contribution.anonymous ? '[checked=\'checked\']' : '') + '[value=\'1\']', {
                onclick: function onclick() {
...
var userSettings = {
    ...
    view: function view(ctrl, args) {
        ...
        }, 'Disable my Citizen Supported account')]))), m$1('div', m$1('.w-container', m$1('.w-row', [m$1('.w-col.w-col-4.w-col-push-4', m$1('input.btn.btn.btn-large[name=\'commit\'][type=\'submit\'][value=\'Save\']')), m$1('.w-col.w-col-4')])))])]), m$1('form.w-hidden', { action: '/en/users/' + user.id, method: 'post', config: ctrl.setDeleteForm }, [m$1('input[name=\'authenticity_token\'][type=\'hidden\'][value=\'' + h.authenticityToken() + '\']'), m$1('input[name=\'_method\'][type=\'hidden\'][value=\'delete\']')])]);
    }
};
...
var teamTotal = {
    ...
    view: function view(ctrl, args) {
        return m$1('#team-total-static.w-section.section-one-column.section.u-margintop-40.u-text-center.u-marginbottom-20', [ctrl.vm.collection().map(function (teamTotal) {
            return m$1('.w-container', [m$1('.w-row', [m$1('.w-col.w-col-2'), m$1('.w-col.w-col-8', [m$1('.fontsize-base.u-marginbottom-30', 'Today we are' + teamTotal.member_count + ' People scattered over ' + teamTotal.total_cities + ' Cities in ' + teamTotal.countries.length + ' Countries (' + teamTotal.countries.toString() + ')! Citizen Supported is independent, open-source, and built with love. Our passion is to build an environment where more and more projects can come to life.'), m$1('.fontsize-larger.lineheight-tight.text-success', 'Our team, together, has already $' + h.formatNumber(teamTotal.total_amount) + ' for ' + teamTotal.total_contributed_projects + ' Business!')]), m$1('.w-col.w-col-2')])]);
        })]);
    }
};
